...
import publishBranchJson from './steps/publishBranchJson';
...
const branchRoot = path.join(buildRootPath, branchFSName);
...
console.log(`  Build Root:            ${buildRootPath}`);
...
async function run() {
    ...
    await publishBranchJson(buildResult.info);
}

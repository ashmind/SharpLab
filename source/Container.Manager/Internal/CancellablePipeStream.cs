...
namespace SharpLab.Container.Manager.Internal {
    public class CancellablePipeStream : Stream {
        private readonly PipeStream _baseStream;
        private bool _cancellationFailed = false;

        public CancellablePipeStream(PipeStream baseStream) {
            if (!baseStream.CanRead)
                throw new NotSupportedException("CancellablePipeStream only supports readable streams at the moment.");
        ...
        public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken) {
            ...
            using var cancellationRegistration = cancellationToken.Register(static thisStreamAsObject => {
                var thisStream = (CancellablePipeStream)thisStreamAsObject!;

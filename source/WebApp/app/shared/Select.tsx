...
export type SelectOptions<TValue> = ReadonlyArray<Option<TValue>|OptionGroup<TValue>>;
...
const renderOptions = <TValue extends string>(options: SelectOptions<TValue>): ReadonlyArray<JSX.Element> => options.map((optionOrGroup, index) => {
...
export const Select = <TValue extends string>({ value, options, onSelect, className, ...htmlProps }: Props<TValue>) => {
    ...
    const onChange = (e: React.ChangeEvent<HTMLSelectElement>) => onSelect(e.target.value as TValue);

...
namespace TryRoslyn.Core.Decompilation.Support {
    public class ILCommentator : ITextOutput {
        private static Dictionary<string, string> _comments;

        private readonly ITextOutput _innerTextOutput;
        private readonly int _commentIndentation;

        private int _currentLineLength;
        private string _comment;

        public TextLocation Location {
            get { return _innerTextOutput.Location; }
        }

        public ILCommentator(ITextOutput innerTextOutput, int commentIndentation) {
            if (innerTextOutput == null)
                throw new ArgumentNullException(nameof(innerTextOutput));

            _innerTextOutput = innerTextOutput;
            _commentIndentation = commentIndentation;

            InitializeComments();
        }

        public void Indent() {
            _innerTextOutput.Indent();
        }

        public void Unindent() {
            _innerTextOutput.Unindent();
        }

        public void Write(char ch) {
            _innerTextOutput.Write(ch);
            _currentLineLength += 1;
        }

        public void Write(string text) {
            _innerTextOutput.Write(text);
            _currentLineLength += text.Length;
        }

        public void WriteLine() {
            WriteComment();
            _innerTextOutput.WriteLine();
            _currentLineLength = 0;
        }

        public void WriteDefinition(string text, object definition, bool isLocal = true) {
            _innerTextOutput.WriteDefinition(text, definition, isLocal);
            _currentLineLength += text.Length;
        }

        public void WriteReference(string text, object reference, bool isLocal = false) {
            SetComment(text);
            _innerTextOutput.WriteReference(text, reference, isLocal);
            _currentLineLength += text.Length;
        }
        ...
        private void SetComment(string text) {
            string comment;
            if (_comments.TryGetValue(text, out comment)) {
                _comment = comment;
            }
        }
        ...
        private void WriteComment() {
            ...
            _innerTextOutput.Write(new string(' ', Math.Max(_commentIndentation - _currentLineLength, 1)));
            _innerTextOutput.Write(string.Format("// {0}", _comment));
            _comment = null;
        }
        ...
        private static void InitializeComments() {
            ...
            _comments = new Dictionary<string, string> {
                ...
                ["callvirt"] = "Call a method associated with an object",
